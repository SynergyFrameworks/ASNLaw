...
namespace Domain.Common
{
    ...
    {
        public static string RegExFixInvalidCharacters(string inputText)
        {
            string pattern = "[\\~#%&*{}/:<>?|\"-]";
            string replacement = " ";

            Regex regEx = new Regex(pattern);
            string sanitized = Regex.Replace(regEx.Replace(inputText, replacement), @"\s+", " ");
        ...
        public static int GetNewUID(DataTable _dt)
        {
            int UID = -1;
            int newUID = -1;
            foreach (DataRow row in _dt.Rows) // Loop thru rows.
            {
        ...
        {
            double d;
            return double.TryParse(value, out d) && !(double.IsNaN(d) || double.IsInfinity(d));
        }
        ...
        public static bool ListItemExists(List<string> lstValue, string xValue) 
        {
 
            foreach (var value in lstValue)
            {
                if (xValue.ToLower() == value.ToLower())
                {
                    return true;
                }
            }
        ...
        public static int CombineLists(ref List<string> lstPrimary, List<string> lstSource)
        {
            int i = 0;
            foreach (string item in lstSource)
            {
                if (!ListItemExists(lstPrimary, item))
                {
                    lstPrimary.Add(item);
                    i++;
                }
