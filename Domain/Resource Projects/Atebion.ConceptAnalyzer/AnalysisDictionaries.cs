...
namespace Atebion.ConceptAnalyzer
{
    public class AnalysisDictionaries
    {
        ...
        private Atebion_Dictionary.Dictionary _dicMgr;
        ...
        public DataSet Get_Documents_Dic_Summary(string ProjectName, string AnalysisName, string[] Docs, string Dictionary, string DictionariesPath, out string SumXRefPathFile, out DataSet dsDicDocsSum4Filtering)
        {
            ...
            _dicMgr = new Atebion_Dictionary.Dictionary(DictionariesPath, Dictionary);
        ...
        public int Dictionary_Analysis(string AnalysisName, string DocumentName, string Dictionary, string DictionariesPath, string DocXMLPath, string DocParseSegPath, bool FindWholeWords, bool FindSynonyms, out int NotFoundDicItemsQty)
        {
            ...
            foreach (DataRow rowParseSeg in dsParseResults.Tables[0].Rows)
            {
                ...
                foreach (DataRow rowDicItem in dsDic.Tables[Atebion_Dictionary.DictionaryFieldConst.TableName].Rows)
                {
                    ...
                    if (FindSynonyms)
                    {
                        ...
                        filter = string.Concat(Atebion_Dictionary.SynonymsFieldConst.Dictionary_UID, " = ", uid_dic);
                        DataRow[] rowSynonyms = dsDic.Tables[Atebion_Dictionary.SynonymsFieldConst.TableName].Select(filter);
                        if (rowSynonyms.Length > 0)
                        {
                            foreach (DataRow rowSynonym in rowSynonyms)
                            {
                                synonymItem = rowSynonym[Atebion_Dictionary.SynonymsFieldConst.Item].ToString();
        ...
        public int DicItemsNotFound(string AnalysisName, string DocumentName, string Dictionary, string DictionariesPath)
        {
            ...
            {
                dicItem = rowDicItem[Atebion_Dictionary.DictionaryFieldConst.Item].ToString();
                ...
                if (rowwithDicItem.Length == 0)                    
                {
                    ...
                    rowNot[DictionariesAnalysisFieldConst.DicItem] = rowDicItem[Atebion_Dictionary.DictionaryFieldConst.Item].ToString();
                    rowNot[DictionariesAnalysisFieldConst.Definition] = rowDicItem[Atebion_Dictionary.DictionaryFieldConst.Definition].ToString();
                    rowNot[DictionariesAnalysisFieldConst.Weight] = Convert.ToDouble(rowDicItem[Atebion_Dictionary.DictionaryFieldConst.Weight].ToString());
