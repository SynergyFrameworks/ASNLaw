...
namespace Atebion.UserCard
{
    public class Manager
    {
        public Manager(string UserDir)
        {
            _UserDir = UserDir;
        }
        ...
        {
            string results = Directories.DirExistsOrCreate(path);
            if (results.Length > 0)
            {
                _ErrorMessage = results;
                return null;
            }
        ...
        public bool ReadUserFile(string file)
        {
            ...
            {
                _ErrorMessage = string.Concat("Unable to file User Card file: ", pathFile);
                return false;
            }
