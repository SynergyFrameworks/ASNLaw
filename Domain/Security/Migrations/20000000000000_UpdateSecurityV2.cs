...
namespace Scion.Business.Security.Migrations
{
    public partial class UpdateSecurityV2 : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(@"IF (EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = '__MigrationHistory'))
                IF (EXISTS (SELECT * FROM __MigrationHistory WHERE ContextKey = 'ScionAnalytics.Foundation.Data.Security.Identity.SecurityDbContext'))
                    BEGIN
