...
namespace Infrastructure.Common.Controllers.Api
{
    ...
    public class SettingController : Controller
    {
        ...
        {
            System.Collections.Generic.IEnumerable<ObjectSettingEntry> result = await _settingsManager.GetObjectSettingsAsync(_settingsManager.AllRegisteredSettings.Select(x => x.Name));
            return Ok(result);
        }
        ...
        {
            SettingsSearchCriteria criteria = new SettingsSearchCriteria
            {
                ModuleId = id,
                Take = int.MaxValue
            };
            GenericSearchResult<ObjectSettingEntry> result = await _settingsSearchService.SearchSettingsAsync(criteria);
        ...
        {
            ObjectSettingEntry result = await _settingsManager.GetObjectSettingAsync(name);
            return Ok(result);
        }
        ...
        {
            ObjectSettingEntry setting = await _settingsManager.GetObjectSettingAsync(name);
            object[] result = null;
            if (setting != null)
            {
        ...
        {
            ObjectSettingEntry result = await _settingsManager.GetObjectSettingAsync(PlatformConstants.Settings.UserInterface.Customization.Name);
            return Ok(result);
        }
    }
}
