...
using Scion.Infrastructure.Core;
using Scion.Infrastructure.Core.Assets;
using Scion.Infrastructure.Core.Common;
using Scion.Infrastructure.Core.Exceptions;
using Scion.Infrastructure.Core.ExportImport;
using Scion.Infrastructure.Core.ExportImport.PushNotifications;
using Scion.Infrastructure.Core.PushNotifications;
using Scion.Infrastructure.Core.Security;
using Scion.Infrastructure.Core.Settings;
using Scion.Infrastructure.Hangfire;

using Permissions = Scion.Infrastructure.Core.PlatformConstants.Security.Permissions;
...
namespace Scion.Infrastructure.Controllers.Api
{
    ...
    public class PlatformExportImportController : Controller
    {
        ...
        private readonly IExportImportManager _exportManager;
        ...
        public PlatformExportImportController(IExportImportManager exportManager, IPushNotificationManager pushNotifier, IBlobStorageProvider blobStorageProvider, IBlobUrlResolver blobUrlResolver,
        ...
        {
            return Ok(_exportManager.GetNewExportManifest(_userNameResolver.GetCurrentUserName()));
        }
        ...
        public ActionResult<PlatformExportManifest> LoadExportManifest([FromQuery]string fileUrl)
        {
            ...
            {
                retVal = _exportManager.ReadExportManifest(stream);
            }
            return Ok(retVal);
        }
        ...
        public async Task SampleDataImportBackgroundAsync(Uri url, string tmpPath, SampleDataImportPushNotification pushNotification, IJobCancellationToken cancellationToken, PerformContext context)
        {
            ...
            try
            {
                ...
                {
                    var manifest = _exportManager.ReadExportManifest(stream);
                    if (manifest != null)
                    {
                        await _exportManager.ImportAsync(stream, manifest, progressCallback, new JobCancellationTokenWrapper(cancellationToken));
                    }
                }
            }
        ...
        public async Task PlatformImportBackgroundAsync(PlatformImportExportRequest importRequest, PlatformImportPushNotification pushNotification, IJobCancellationToken cancellationToken, PerformContext context)
        {
            ...
            try
            {
                ...
                using (var stream = new FileStream(localPath, FileMode.Open))
                {
                    ...
                    await _exportManager.ImportAsync(stream, manifest, progressCallback, cancellationTokenWrapper);
                }
            }
            catch (JobAbortedException)
            {
        ...
        public async Task PlatformExportBackgroundAsync(PlatformImportExportRequest exportRequest, PlatformExportPushNotification pushNotification, IJobCancellationToken cancellationToken, PerformContext context)
        {
            ...
            try
            {
                ...
                using (var stream = System.IO.File.OpenWrite(localTmpPath))
                {
                    ...
                    await _exportManager.ExportAsync(stream, manifest, progressCallback, new JobCancellationTokenWrapper(cancellationToken));
