...
using Scion.Business.Security.Authorization;
//using Microsoft.AspNetCore.Authentication.AzureAD.UI;
using Scion.Data.Common.Extensions;
using Scion.DistributedLock;
using Scion.Hangfire;
//using Scion.Infrastructure.Assets.AzureBlobStorage;
//using Scion.Infrastructure.Assets.AzureBlobStorage.Extensions;
//using Scion.Infrastructure.Assets.FileSystem;
//using Scion.Infrastructure.Assets.FileSystem.Extensions;
...
using Scion.Infrastructure.JsonConverters;
using Scion.Infrastructure.Localizations;
using Scion.Infrastructure.Modularity;
using Scion.Infrastructure.Modules;
using Scion.Infrastructure.Security;
using Scion.Infrastructure.Security.Repositories;
using Scion.Infrastructure.Web.Azure;
using Scion.Infrastructure.Web.Extensions;
using Scion.Infrastructure.Web.Infrastructure;
using Scion.Infrastructure.Web.Licensing;
using Scion.Infrastructure.Web.Middleware;
using Scion.Infrastructure.Web.Migrations;
using Scion.Infrastructure.Web.PushNotifications;
using Scion.Infrastructure.Web.Redis;
using Scion.Infrastructure.Web.Security;
using Scion.Infrastructure.Web.Security.Authentication;
...
namespace Scion.Infrastructure.Web
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddOptions<PlatformOptions>().Bind(Configuration.GetSection("ScionAnalytics")).ValidateDataAnnotations();
            ...
            IMvcBuilder mvcBuilder = services.AddMvc(mvcOptions =>
            {
                ...
                HttpNoContentOutputFormatter noContentFormatter = mvcOptions.OutputFormatters.OfType<HttpNoContentOutputFormatter>().FirstOrDefault();
            ...
            {
                IOptions<MvcNewtonsoftJsonOptions> serv = js.GetService<IOptions<MvcNewtonsoftJsonOptions>>();
                return JsonSerializer.Create(serv.Value.SerializerSettings);
            });

            services.AddDbContext<SecurityDbContext>(options =>
            {
                options.UseSqlServer(Configuration.GetConnectionString("ScionAnalytics"));
            ...
            JwtSecurityTokenHandler defaultTokenHandler = new JwtSecurityTokenHandler();
            ...
            authBuilder.AddJwtBearer(options =>
            {
                ...
                {
                    X509Certificate2 publicCert = new X509Certificate2(Configuration["Auth:PublicCertPath"]);
            ...
            IConfigurationSection azureAdSection = Configuration.GetSection("AzureAd");

            if (azureAdSection.GetChildren().Any())
            {
                AzureAdOptions options = new Scion.Infrastructure.Web.Azure.AzureAdOptions();
            ...
            services.AddOptions<Scion.Infrastructure.Security.AuthorizationOptions>().Bind(Configuration.GetSection("Authorization")).ValidateDataAnnotations();
            Scion.Infrastructure.Security.AuthorizationOptions authorizationOptions = Configuration.GetSection("Authorization").Get<Scion.Infrastructure.Security.AuthorizationOptions>();
            PlatformOptions platformOptions = Configuration.GetSection("ScionAnalytics").Get<PlatformOptions>();
            ...
            services.AddOpenIddict()
                ...
                }).AddServer(options =>
                {
                    ...
                    OpenIddictServerAspNetCoreBuilder builder = options.UseAspNetCore().
                    ...
                    //byte[] bytes = File.ReadAllBytes(Configuration["Auth:PrivateKeyPath"]);
            ...
            services.AddAuthorization(options =>
            {
                ...
                AuthorizationPolicy mutipleSchemaAuthPolicy = new AuthorizationPolicyBuilder().AddAuthenticationSchemes(JwtBearerDefaults.AuthenticationScheme, ApiKeyAuthenticationOptions.DefaultScheme)
            ...
            services.AddSingleton<IAuthorizationPolicyProvider, Scion.Infrastructure.Security.Authorization.PermissionAuthorizationPolicyProvider>();
            ...
            services.AddOptions<LocalStorageModuleCatalogOptions>().Bind(Configuration.GetSection("ScionAnalytics"))
            ...
            // AI modules initialization   https://github.com/microsoft/ApplicationInsights-dotnet/issues/2114 until we don't
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
        {
            ...
            Dictionary<string, string> fileExtensionsBindings = new Dictionary<string, string>()
            {
                { ".liquid", "text/html"},
                { ".md", "text/html"}
            };
            ...
            // Add custom bindings
            foreach (KeyValuePair<string, string> binding in fileExtensionsBindings)
            {
            ...
            IEnumerable<ModuleInfo> localModules = app.ApplicationServices.GetRequiredService<ILocalModuleCatalog>().Modules;
            foreach (ManifestModuleInfo module in localModules.OfType<ManifestModuleInfo>())
            {
                app.UseStaticFiles(new StaticFileOptions()
                {
            ...
            IOptions<MvcNewtonsoftJsonOptions> mvcJsonOptions = app.ApplicationServices.GetService<IOptions<MvcNewtonsoftJsonOptions>>();
