...
namespace Mvc.Server
{
    public class AuthorizationController : Controller
    {
        ...
        public async Task<ActionResult> Exchange()
        {
            ...
            {
                ApplicationUser user = await _userManager.FindByNameAsync(openIdConnectRequest.Username);
                ...
                Microsoft.AspNetCore.Identity.SignInResult result = await _signInManager.CheckPasswordSignInAsync(user, openIdConnectRequest.Password, lockoutOnFailure: true);
                if (!result.Succeeded)
                {
                    return BadRequest(new OpenIddictResponse
                    {
                ...
                AuthenticationTicket ticket = await CreateTicketAsync(openIdConnectRequest, user);
                ClaimsPrincipal claimsPrincipal = await _userClaimsPrincipalFactory.CreateAsync(user);
                string[] limitedPermissions = _authorizationOptions.LimitedCookiePermissions?.Split(PlatformConstants.Security.Claims.PermissionClaimTypeDelimiter, StringSplitOptions.RemoveEmptyEntries) ?? new string[0];
            ...
            else if (openIdConnectRequest.IsRefreshTokenGrantType())
            {
                ...
                AuthenticateResult info = await HttpContext.AuthenticateAsync(OpenIddictServerAspNetCoreDefaults.AuthenticationScheme);
                ...
                AuthenticationTicket ticket = await CreateTicketAsync(openIdConnectRequest, user, info.Properties);
                return SignIn(ticket.Principal, ticket.Properties, ticket.AuthenticationScheme);
            }
            else if (openIdConnectRequest.IsClientCredentialsGrantType())
            {
                ...
                OpenIddictEntityFrameworkCoreApplication application = await _applicationManager.FindByClientIdAsync(openIdConnectRequest.ClientId, HttpContext.RequestAborted);
                if (application == null)
                {
                    return BadRequest(new OpenIddictResponse
                    {
                ...
                // Create a new authentication ticket.
                AuthenticationTicket ticket = CreateTicket(application);
                return SignIn(ticket.Principal, ticket.Properties, ticket.AuthenticationScheme);
            }
