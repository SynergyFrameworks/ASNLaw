...
using Scion.Infrastructure.Core.Events;
using Scion.Infrastructure.Core.Security;
using Scion.Infrastructure.Core.Security.Events;
using Scion.Infrastructure.Model.Security;

namespace Scion.Infrastructure.Controllers
{
    ...
    public class ExternalSignInController : Controller
    {
        ...
        public async Task<ActionResult> SignInCallback(string returnUrl)
        {
            ...
            if (!externalLoginResult.Succeeded)
            {
                ...
                //first - when the PFA platform user account already exists, it is just missing an external login info and
                ...
                if (platformUser == null)
                {
                    ...
                    if (!result.Succeeded)
                    {
                        ...
                        throw new InvalidOperationException("Failed to save a PFA platform account due the errors: " + joinedErrors);
                    }
                }
